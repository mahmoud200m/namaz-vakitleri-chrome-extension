...
function updateCountries(callback){
	var url = "http://t-services.mobilion.com.tr/DiyanetServicePub/DynNamazVakitService.asmx/GetUlkeler?filter=";
	invokeGetRequest(url, function(response, isError){
		var countries = [];
		if (!isError) {
			...
			var map = new HashMap();
			var country_names = [];
			$("ArrayOfUlkeItem UlkeItem", xmlDoc).each(function(){
				var code = $("ulkeKodu", this).text();
...
function updateProvinces(a, callback){
	var url = "http://t-services.mobilion.com.tr/DiyanetServicePub/DynNamazVakitService.asmx/GetEyaletler?filter=&ulkeKodu=" + a;
	invokeGetRequest(url, function(response, isError){
		var provinces = [];
		if (!isError) {
			var xmlDoc = $.parseXML(response);
			$("ArrayOfEyaletItem EyaletItem", xmlDoc).each(function(){
				var ccode = $("ulkeKodu", this).text();
				var code = $("eyaletKodu", this).text();
				var value = $("eyaletAdi", this).text();
				provinces.push({code: code, value: value, countryCode: ccode});
			});
		}
...
function updateCities(a, b, callback){
	if (b == null)
	b = "0";
	var url = "http://t-services.mobilion.com.tr/DiyanetServicePub/DynNamazVakitService.asmx/GetSehirler?filter=&ulkeKodu=" + a + "&eyaletKod=" + b;
	invokeGetRequest(url, function(response, isError){
		var cities = [];
		if (!isError) {
			var xmlDoc = $.parseXML(response);
			$("ArrayOfSehirItem SehirItem", xmlDoc).each(function(){
				var ccode = $("ulkeKodu", this).text();
				var ecode = $("eyaletKodu", this).text();
				var evalue = $("sehirAdi", this).text();
				var code = $("sehirId", this).text();
				var value = $("sehirAlias", this).text();
				if (value=="Merkez") {
					value=chrome.i18n.getMessage("city_center");
				}
				var latitude = $("enlem", this).text();
				var longitude = $("boylam", this).text();
				cities.push({code: code, value: value, countryCode: ccode, provinceCode: ecode, provinceName: evalue, latitude:latitude, longitude:longitude });
			});
		}
...
function updateTimes(a, callback){
	var url = "http://t-services.mobilion.com.tr/DiyanetServicePub/DynNamazVakitService.asmx/GetVakitler?saatFarki=0&sehirId=" + a;
	invokeGetRequest(url, function(response, isError){
		var times = new HashMap();
		if (!isError) {
			var xmlDoc = $.parseXML(response);
			$("ArrayOfDateItem DateItem", xmlDoc).each(function(){
				var date = $("gun", this).text();
				...
				var qibla = $("kible", this).text().split(" ");
